...
async function handleAuthRedirect(){
  ...
  const type = p.get('type'); // 'recovery' bei Passwort-Reset
  ...
  // Bei Passwort-Reset: direkt zur Passwort-Seite, nicht zum Barber-Dashboard
  if (type === 'recovery'){
...
(async function boot(){
  ...
  sb.auth.onAuthStateChange(async (ev) => {
    if (ev === 'SIGNED_IN' || ev === 'SIGNED_OUT' || ev === 'TOKEN_REFRESHED'){
      await loadSession();
      // WICHTIG: keine Zwangsumleitung! Supabase feuert SIGNED_IN auch beim
      // Wiederherstellen der Sitzung und beim Token-Refresh — eine Umleitung
      // würde sonst mitten im Kunden-Buchungsflow (z. B. auf der Zahlungsseite)
      // abbrechen. Neu gezeichnet wird nur, wenn man ohnehin im Barber-/Admin-Bereich ist.
