  ...
  try {
    ...
    app.innerHTML = topbar('Deine Buchung', 'my') + `<div class="view">
      ...
      ${b.status==='booked' && s.type!=='mobile' ? `<div class="card"><div style="font-weight:700;margin-bottom:6px">📍 Vor Ort einchecken</div><div class="note" style="margin-top:0">Scanne den QR-Code an der Rezeption mit deiner Handy-Kamera — oder tippe den Shop-Code ein. Möglich ab 15 Minuten vor deinem Termin.</div>
        <div class="row" style="margin-top:10px"><input id="scode" placeholder="Shop-Code" style="text-transform:uppercase" value="${(() => { try { return new URLSearchParams(location.search).get('c') || ''; } catch { return ''; } })()}" ${canCheckin?'':'disabled'}><button class="btn small ${canCheckin?'':'sec2'}" ${canCheckin?'':'disabled'} onclick="AF.checkin('${b.code}')">Einchecken</button></div>
        ${!canCheckin && b.status === 'booked' ? `<div class="note">🔒 Check-in nach dem Termin nicht mehr möglich.</div>` : ''}<div id="cerr"></div></div>` : ''}
...
async function checkin(code, shopCodeFromUrl){
  ...
  if (!sc){ toast('Bitte Shop-Code eingeben', true); return; }
  ...
  catch(e){ const box = $('#cerr'); if (box) box.innerHTML = `<div class="err">${esc(errMsg(e))}</div>`; else toast(errMsg(e), true); }
}
...
async function saveUnavail(){ const { error } = await sb.from('shops').update({ unavail: myShop.unavail }).eq('id', myShop.id); if (error){ toast(errMsg(error), true); return false; } return true; }
...
async function render(){
  ...
  try {
    ...
    if (parts[0]==='my'){
      // Auto-Checkin: wenn ?c=SHOPCODE in der URL steht (QR-Code gescannt)
      const urlParams = new URLSearchParams(location.search);
      const scFromUrl = urlParams.get('c');
      if (scFromUrl){
        // Alle gespeicherten Buchungscodes durchgehen und versuchen einzuchecken
        const codes = myCodes();
        if (codes.length){
          ...
          if (!checkedIn){ viewMy(); toast('Kein offener Termin für diesen Shop gefunden — prüfe deinen Buchungscode', true); }
          return;
        }
      }
      return viewMy();
...
window.AF = { pickService, afterService, pickStaff, filterStaff, setQty, day, pickTime, book, checkin, cancel, dispute, sendLink, pwLogin, signUp, logout, showResetForm, sendReset, confirmReset, f: fset, rf, togSpec, addArea, sv, addSv, rmSv, st, addSt, rmSt, saveShop, dash: viewDash, dday, setStatus, depPaid, arrived, avTab, togHour, togDay, editShop, connectStripe, shopStatus, resolve, toast, setCity, useLocation, sendChatCustomer, sendChatBarber, statsRange: statsSetRange, statsSearch, adminTab: adminSetTab, adminRange: adminSetRange, adminSearch, setStaffPassword, inviteStaff, toggleStaffPerm };
