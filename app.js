...
async function sendReset(){
  ...
  try {
    const { error } = await sb.auth.resetPasswordForEmail(email, { redirectTo: location.origin + location.pathname + '#/reset-password' });
    if (error) throw error;
...
function viewResetPassword(){
  if (!session){
    app.innerHTML = topbar('Neues Passwort', 'barber') + `<div class="view">
      <div class="warnbox">⏳ Deine Sitzung ist abgelaufen oder der Link wurde bereits verwendet. Bitte fordere einen neuen Link an.</div>
      <button class="btn" style="margin-top:12px" onclick="AF.showResetForm()">Neuen Link anfordern</button></div>`;
    return;
  }
...
async function handleAuthRedirect(){
  ...
  // Fehler von Supabase abfangen (z. B. abgelaufener Link)
  if (h.includes('error=')){
...
(async function boot(){
  ...
  sb.auth.onAuthStateChange(async (ev) => {
    if (ev === 'PASSWORD_RECOVERY'){
      // Supabase hat den Reset-Token automatisch verarbeitet — Session steht bereit
      await loadSession();
