...
// ---------- Marketing-Banner für Gäste ----------
function getGuestBanner(){
  const images = (C.HERO_IMAGES && C.HERO_IMAGES.length) ? C.HERO_IMAGES : [];
  const image = images.length ? images[Math.floor(Math.random() * images.length)] : '';
  const slogans = [
    "Der Look, der dich verändert.",
    "Deine Zeit. Dein Stil. Dein Barber.",
    "Kein Warten. Kein Chaos. Nur du und dein Barber.",
    "Der Barber, der dich kennt."
  ];
...
function topbar(title, back, right){
  const guestBanner = (!session && back) ? (() => {
    const b = getGuestBanner();
    return b.image ? `<div class="guest-banner" style="background-image:url('${esc(b.image)}')"><span>${b.slogan}</span></div>` : '';
  })() : '';
  return `<div class="topbar">${back ? `<div class="back" onclick="location.hash='#/${back}'">←</div>` : `<div class="logo">AFROCUTS${C.TEST_MODE ? '<span class="testtag">TEST ' + APP_VERSION + '</span>' : ''}</div>`}
    ${title ? `<div class="title">${esc(title)}</div>` : ''}<div>${right || (back ? '<div style="width:34px"></div>' : '')}</div></div>${guestBanner}`;
}
function bottomNav(active){
  ...
  if (!session){
    const b = getGuestBanner();
    if (b.image) guestFooter = `<div class="guest-banner" style="background-image:url('${esc(b.image)}')"><span>${b.slogan}</span></div>`;
  }
  return `${guestFooter}<div class="nav"><div class="in">${it('home','🗺️','Barber')}${it('my','📅','Meine Termine')}${it('barber','💈','Für Barber')}</div></div>
    <div class="credits">Bilder von <a href="https://unsplash.com/@ato_aikins" target="_blank">Ato Aikins</a>, <a href="https://unsplash.com/@obionyeador" target="_blank">Obi</a>, <a href="https://unsplash.com/@bangscreative" target="_blank">Mohamed B.</a> und <a href="https://unsplash.com/@kawerodrigues" target="_blank">Kawe Rodrigues</a> auf <a href="https://unsplash.com" target="_blank">Unsplash</a></div>`;
}
...
function startHeroRotation(images){
  ...
  heroTimer = setInterval(() => {
    const a = document.getElementById('heroLayerA'), b = document.getElementById('heroLayerB');
    if (!a || !b) { clearInterval(heroTimer); return; }
    idx = (idx + 1) % images.length;
    const showingA = a.classList.contains('on');
    const nextLayer = showingA ? b : a;
    nextLayer.style.backgroundImage = `url('${images[idx]}')`;
    nextLayer.classList.add('on');
    (showingA ? a : b).classList.remove('on');
  }, 5000);
}
async function viewHome(){
  const loggedInBanner = session ? `<div class="lock">✓ Du bist als Barber angemeldet. <b style="color:var(--gold-l);cursor:pointer" onclick="location.hash='#/barber'">Zum Dashboard →</b> <span style="color:#e2958f;cursor:pointer;margin-left:8px" onclick="AF.logout()">Abmelden</span></div>` : '';
  const heroImgs = (C.HERO_IMAGES && C.HERO_IMAGES.length) ? C.HERO_IMAGES : (C.HERO_IMAGE ? [C.HERO_IMAGE] : []);
  app.innerHTML = topbar() + `<div class="view">
    <div class="hero2${heroImgs.length ? ' has-bg' : ''}" id="heroBox">
      ${heroImgs.length ? `<div class="hero-bg-layer on" id="heroLayerA" style="background-image:url('${esc(heroImgs[0])}')"></div><div class="hero-bg-layer" id="heroLayerB"></div>` : ''}
      <div class="h1b">Der Look, der dich verändert.<br>Der Barber, der ihn kennt.</div><p>Entdecke die besten Afro-Barber in deiner Stadt — verbindlich buchbar, ohne Anmeldung.</p></div>
  ...
  if (heroImgs.length > 1) startHeroRotation(heroImgs);
  try {
...
// ---------- Buchungs-Flow-State ----------
const B = { shop:null, service:null, staff:'any', dayIdx:0, time:null, staffIdx:null, qty:1 };
...
async function viewShop(id){
  ...
  try {
    ...
    app.innerHTML = topbar('', 'home') + `<div class="view">
      ${s.portfolio_image ? `<div class="profile-hero"><img src="${esc(s.portfolio_image)}" onerror="this.parentElement.style.display='none'"></div>` : heroBanner('Dein neuer Look beginnt hier.', 'Wähl deinen Service — den Rest macht dein Barber.')}
      ...
      <div class="sec" style="color:var(--gold-l);font-size:14px;letter-spacing:.2px;text-transform:none">Bereit für deine Verwandlung?</div>
...
function afterService(){ B.staff = 'any'; nav('time'); }
...
function getHolidays(year){
  ...
  const d = (off) => { const r = new Date(e); r.setDate(r.getDate()+off); return dkey(r); };
  return new Set([
    year+'-01-01',                    // Neujahr
    d(-2),                            // Karfreitag
    d(0),                             // Ostersonntag
...
function isHoliday(date){
  ...
  return _holidayCache[y].has(dkey(date));
}
function holidayName(date){
  const dk = dkey(date), y = date.getFullYear();
  const e = easterSunday(y);
  const d = (off) => { const r = new Date(e); r.setDate(r.getDate()+off); return dkey(r); };
  const names = { [y+'-01-01']:'Neujahr', [d(-2)]:'Karfreitag', [d(0)]:'Ostersonntag', [d(1)]:'Ostermontag',
    [y+'-05-01']:'Tag der Arbeit', [d(39)]:'Christi Himmelfahrt', [d(49)]:'Pfingstsonntag', [d(50)]:'Pfingstmontag',
    [y+'-10-03']:'Tag der Dt. Einheit', [y+'-12-25']:'1. Weihnachtstag', [y+'-12-26']:'2. Weihnachtstag' };
  return names[dk] || null;
}

function computeSlots(shop, date, duration, taken){
  // Feiertage automatisch sperren (Kunden sehen keine Slots)
...
async function viewTime(){
  if (!B.shop || !B.service) return nav('home');
  B._heroImg = heroImgPick();
  app.innerHTML = topbar('Termin wählen', 'shop/' + B.shop.id) + `<div class="view" id="timeView"><div class="empty" style="padding:40px 0"><span class="spinner"></span> Lade …</div></div>`;
  await loadDaySlots();
}
async function loadDaySlots(){
  ...
  try {
    ...
    if (error) throw error;
    B._per = computeSlots(B.shop, d, B.service.duration, data || []);
    B._date = dkey(d);
    renderTimeView();
  } catch(e){ $('#timeView').innerHTML = `<div class="empty">Fehler: ${esc(errMsg(e))}</div>`; }
}
function renderTimeView(){
  const box = $('#timeView'); if (!box) return;
  const d = fromKey(B._date);
  const multi = (B.shop.staff||[]).length > 1;
  const staffChips = multi ? `<div class="chips" style="margin-bottom:6px">
      <span class="chip ${B.staff==='any'?'on':''}" onclick="AF.filterStaff('any')">Beliebig</span>
      ${B.shop.staff.map((x,i)=>`<span class="chip ${B.staff===String(i)?'on':''}" onclick="AF.filterStaff('${i}')">${esc(x.name)}</span>`).join('')}
    </div>` : '';
  const qtyRow = `<div class="chips" style="margin-bottom:8px"><span style="font-size:12px;color:var(--muted);margin-right:6px">Personen:</span>${[1,2,3,4,5].map(n => `<span class="chip ${B.qty===n?'on':''}" onclick="AF.setQty(${n})">${n}</span>`).join('')}</div>`;
  let slotsHtml;
  if (B._per === null){
    const hn = holidayName(d);
    slotsHtml = `<div class="empty">🔒 ${hn ? esc(hn) + ' — gesetzlicher Feiertag' : 'Geschlossen an diesem Tag'}<br>Bitte einen anderen Tag wählen</div>`;
  } else {
    const per = B._per;
    const step = B.shop.slot_step || 30;
    let times;
    if (B.staff === 'any'){ const set = new Set(); Object.values(per).forEach(a => a.forEach(t => set.add(t))); times = [...set].sort(); }
    else times = per[+B.staff] || [];
    // Bei qty > 1: nur Startzeiten zeigen, wo genug aufeinanderfolgende Slots frei sind
    if (B.qty > 1 && times.length){
      times = times.filter(startT => {
        const startMin = t2m(startT);
        for (let i = 1; i < B.qty; i++){
          const needed = m2t(startMin + i * B.service.duration);
          if (!times.includes(needed)) return false;
        }
        return true;
      });
    }
    if (!times.length){
      slotsHtml = `<div class="empty">📭 ${B.qty > 1 ? `Keine ${B.qty} aufeinanderfolgenden Termine frei` : 'Keine freien Termine an diesem Tag'}${B.staff!=='any' && multi ? ' bei dieser Person' : ''}<br>${B.qty > 1 ? 'Probier weniger Personen oder einen anderen Tag' : (B.staff!=='any' && multi ? 'Tippe oben „Beliebig" oder einen anderen Namen — oder schau beim nächsten Tag vorbei' : 'Schau beim nächsten Tag vorbei')}</div>`;
    } else {
      slotsHtml = times.map(t => {
        const sn = B.staff === 'any' ? Object.keys(per).map(Number).filter(si => per[si].includes(t)).map(si => B.shop.staff[si]?.name).filter(Boolean)[0] : null;
        const endMin = t2m(t) + B.qty * B.service.duration;
        const label = B.qty > 1 ? `${t} – ${m2t(endMin)}` : t;
        return `<div class="opt ${B.time===t?'on':''}" onclick="AF.pickTime('${t}')">${label}${B.qty>1?`<span style="display:block;font-size:9.5px;opacity:.7;font-weight:400">${B.qty}× ${esc(B.service.name)}</span>`:''}${sn && multi && B.qty===1 ? `<span style="display:block;font-size:9.5px;opacity:.7;font-weight:400">${esc(sn)}</span>` : ''}</div>`;
      }).join('');
    }
  }
  const totalPrice = B.service.price * B.qty;
  const totalDur = B.service.duration * B.qty;
  box.innerHTML = `${heroBanner('Wann passt es dir?', 'Freie Slots gehen schnell weg — schnapp dir deinen.', B._heroImg)}
    ${staffChips}${qtyRow}
    ...
    <div class="sec">Freie Uhrzeiten</div><p style="color:var(--muted);font-size:12px;margin:-4px 0 8px">${B.qty>1?`${B.qty} Personen × ${B.service.duration} Min. = ${totalDur} Min. am Stück benötigt`:'Jeder freie Slot ist echt — sichere ihn dir, bevor es jemand anders tut.'}</p><div class="slots" id="slots">${slotsHtml}</div>
    <div class="summary" id="sum"><b>${esc(B.shop.name)}</b> · ${B.qty > 1 ? B.qty + '× ' : ''}${esc(B.service.name)} · ${totalDur} min${B.qty>1?' · '+eur(totalPrice):''}<br>${B.time ? fmtD(d) + ' · ' + B.time + ' Uhr' + (multi && B.shop.staff[B.staffIdx]?.name ? ' · bei ' + esc(B.shop.staff[B.staffIdx].name) : '') : 'Wähle Tag &amp; Uhrzeit'}</div><div style="height:80px"></div>
    <div class="sticky"><div class="in"><button class="btn" id="toPay" ${B.time?'':'disabled'} onclick="location.hash='#/pay'">Weiter zur Zahlung</button></div></div>`;
}
function setQty(n){ B.qty = n; B.time = null; B.staffIdx = null; renderTimeView(); }
function filterStaff(v){ B.staff = v; B.time = null; B.staffIdx = null; renderTimeView(); }
function day(n){ const x = B.dayIdx + n; if (x < 0 || x > 13) return; B.dayIdx = x; B.time = null; loadDaySlots(); }
function pickTime(t){
  B.time = t;
...
function miniHero(){
  const imgs = (C.HERO_IMAGES && C.HERO_IMAGES.length) ? C.HERO_IMAGES : (C.HERO_IMAGE ? [C.HERO_IMAGE] : []);
  if (!imgs.length) return '';
  const pick = imgs[Math.floor(Math.random() * imgs.length)];
  return `<div class="mini-hero"><img src="${esc(pick)}" loading="lazy" onerror="this.closest('.mini-hero').style.display='none'"></div>`;
}
function heroImgPick(){
  const imgs = (C.HERO_IMAGES && C.HERO_IMAGES.length) ? C.HERO_IMAGES : (C.HERO_IMAGE ? [C.HERO_IMAGE] : []);
  return imgs.length ? imgs[Math.floor(Math.random() * imgs.length)] : '';
}
function heroBanner(headline, sub, fixedImg){
  const pick = fixedImg || heroImgPick();
  if (!pick) return `<div class="sec" style="color:var(--gold-l);font-size:15px;text-transform:none;letter-spacing:0">${headline}</div>`;
  return `<div class="hero-banner" style="background-image:url('${esc(pick)}')">
    <div class="hb-text"><b>${headline}</b>${sub ? `<span>${sub}</span>` : ''}</div></div>`;
}
function viewPay(){
  if (!B.shop || !B.service || !B.time){
    app.innerHTML = topbar('Anzahlung', 'home') + `<div class="view">
      <div class="warnbox">⏳ Deine Auswahl ist nicht mehr aktiv — das passiert, wenn die Seite neu geladen oder zu lange offen gelassen wurde.</div>
      <p class="note">Kein Problem: Wähl deinen Barber und Termin einfach noch einmal, es dauert nur einen Moment.</p>
      <button class="btn" style="margin-top:12px" onclick="location.hash='#/home'">Zurück zur Barber-Suche</button></div>` + bottomNav('home');
    return;
  }
  const totalPrice = B.service.price * B.qty;
  const dep = Math.round(totalPrice * B.shop.deposit_pct) / 100;
  const rest = totalPrice - dep;
  const d = fromKey(B._date);
  const endTime = B.qty > 1 ? m2t(t2m(B.time) + B.qty * B.service.duration) : null;
  app.innerHTML = topbar('Anzahlung', 'time') + `<div class="view">
    ${miniHero()}
    <div class="h2" style="margin-bottom:2px">Fast geschafft.</div><p style="color:var(--muted);font-size:12.5px;margin:0 0 12px">Sichere dir deinen Platz — in wenigen Sekunden.</p>
    <div class="card"><div class="prow"><span>Barber</span><b>${esc(B.shop.name)}</b></div><div class="prow"><span>Service</span><b>${B.qty > 1 ? B.qty + '× ' : ''}${esc(B.service.name)}</b></div><div class="prow"><span>Termin</span><b>${fmtD(d)} · ${B.time}${endTime ? ' – ' + endTime : ''} Uhr</b></div><div class="prow"><span>Gesamtpreis</span><b>${eur(totalPrice)}</b></div><div class="hr"></div>
    ...
    ${B.shop.type==='mobile' ? `<div class="note" style="color:var(--gold-l);margin-bottom:2px">Kein Warten mit Kindern im Laden — du bekommst deine Zeit zurück.</div><div class="sec">🚗 Deine Adresse für den Hausbesuch</div><input id="addr" value="${esc((() => { try { return localStorage.getItem('afro_caddr')||''; } catch { return ''; } })())}" placeholder="Straße, Hausnummer, PLZ, Etage">` : ''}
    <div class="sec">Deine Kontaktdaten</div>
    <label class="l">Name</label><input id="cname" value="${esc((() => { try { return localStorage.getItem('afro_cname')||''; } catch { return ''; } })())}" placeholder="Vor- und Nachname" autocomplete="name">
    <label class="l">Handynummer oder E-Mail</label><input id="ccontact" value="${esc((() => { try { return localStorage.getItem('afro_ccontact')||''; } catch { return ''; } })())}" placeholder="Für Buchungsbestätigung — der Barber schreibt dir über den Chat" autocomplete="tel">
    <div class="note" style="margin-top:-8px">💬 Der Barber kontaktiert dich ausschließlich über den Chat in der App, nie per Anruf.</div>
...
async function book(){
  ...
  try {
    // Kundendaten merken für Folgebuchungen
    try { localStorage.setItem('afro_cname', name); localStorage.setItem('afro_ccontact', contact); if (addr) localStorage.setItem('afro_caddr', addr); } catch {}

    if (B.qty > 1){
      // Gruppenbuchung: alle Slots atomar in einer Transaktion
      const times = [];
      const startMin = t2m(B.time);
      for (let i = 0; i < B.qty; i++) times.push(m2t(startMin + i * B.service.duration));
      const { data, error } = await sb.rpc('create_group_booking', { p_shop: B.shop.id, p_staff_index: B.staffIdx || 0, p_staff_name: (B.shop.staff||[])[B.staffIdx||0]?.name || '', p_service: B.service.name, p_duration: B.service.duration, p_price: B.service.price, p_date: B._date, p_times: times, p_name: name, p_contact: contact, p_address: addr });
      if (error) throw error;
      (data.codes || []).forEach(c => saveCode({ code: c, shop: B.shop.name, date: B._date, time: times[data.codes.indexOf(c)] || B.time }));
      if (data.payment_link){ window.open(data.payment_link, '_blank'); }
      nav('b/' + data.codes[0] + '?new=1&group=' + data.count);
    } else {
      // Einzelbuchung (bestehende Logik)
...
function viewMy(){
  const codes = myCodes();
  app.innerHTML = topbar() + `<div class="view">${miniHero()}<div class="h2">Meine Termine</div><p style="color:var(--gold-l);font-size:12.5px;margin:-6px 0 12px">Dein Style, deine Zeit, immer im Blick.</p>
...
const STATUS_BARBER = { booked:['● Reserviert',''], checked_in:['✓ Eingecheckt',''], completed:['✓ Abgeschlossen',''], cancelled_customer:['Kunde hat storniert','red'], cancelled_barber:['Du hast abgesagt','red'], no_show_barber:['Barber nicht erschienen','red'], no_show_customer:['Nicht erschienen','red'] };
async function viewBooking(code, isNew, justPaid, groupCount){
  ...
  try {
    ...
    const now = new Date(); const winOpen = b.status === 'booked' && now <= new Date(start.getTime() + 60*60000);
    const canCheckin = winOpen;
    const canDispute = ['booked','no_show_barber'].includes(b.status) && now > start;
    app.innerHTML = topbar('Deine Buchung', 'my') + `<div class="view">
      ${!session ? heroBanner('Dein Termin, dein Moment.', 'Alles Wichtige auf einen Blick.') : ''}
      ${isNew ? `<div class="check">✓</div><div class="center h2">${groupCount > 1 ? groupCount + ' Termine auf einmal gesichert!' : 'Gesichert. Dein Moment gehört dir.'}</div><div class="center note" style="margin-bottom:14px">${groupCount > 1 ? 'Alle ' + groupCount + ' Codes findest du unter „Meine Termine". Keiner kann dir dazwischen etwas wegschnappen.' : 'Merk dir deinen Code — damit findest du die Buchung auf jedem Gerät wieder.'}</div><button class="btn sec2" style="margin-top:8px" onclick="location.hash='#/shop/${s.id}'">Noch einen Termin beim selben Barber buchen</button>` : ''}
      ...
      ${b.status==='booked' && s.type!=='mobile' ? `<div class="card">
        <div style="font-weight:700;margin-bottom:6px">📍 Vor Ort einchecken</div>
        <button class="btn" style="width:100%;margin-top:8px" ${canCheckin?'':'disabled'} onclick="AF.openScanner('${b.code}')">📷 QR-Code scannen</button>
        <div class="note" style="margin-top:6px;text-align:center">Halte die Kamera auf das QR-Poster an der Rezeption.</div>
        ${!canCheckin && b.status === 'booked' ? '<div class="note">🔒 Check-in nach dem Termin nicht mehr möglich.</div>' : ''}
        <div id="cerr"></div></div>` : ''}
      ...
      ${b.status==='checked_in' ? '<div class="lock">✓ Angekommen. Lehn dich zurück — jetzt übernimmt dein Barber.<br><span style="opacity:.8;font-size:11.5px">Deine Anzahlung wird nach dem Termin freigegeben.</span></div>' : ''}
...
async function qrCheckin(shopCode){
  const bookCode = ($('#qrBookCode') ? $('#qrBookCode').value.trim().toUpperCase() : '');
  const box = $('#qrErr');
  if (!bookCode || bookCode.length < 4){ if (box) box.innerHTML = '<div class="err">Bitte deinen 6-stelligen Buchungscode eingeben.</div>'; return; }
  try {
    const { error } = await sb.rpc('checkin_booking', { p_code: bookCode, p_shop_code: shopCode.toUpperCase() });
    if (error) throw error;
    toast('✓ Angekommen — dein Moment beginnt!');
    nav('b/' + bookCode);
  } catch(e){
    let msg = errMsg(e);
    if (msg.includes('Falscher Shop-Code')) msg = 'Dieser Buchungscode gehört zu einem anderen Laden — bist du im richtigen?';
    if (msg.includes('nicht gefunden')) msg = 'Buchungscode nicht gefunden — prüfe die 6 Zeichen nochmal.';
    if (box) box.innerHTML = `<div class="err">${esc(msg)}</div>`;
  }
}
// ---------- QR-Scanner für Check-in ----------
let _scannerModal = null, _scannerStream = null, _scannerInterval = null;
async function openScanner(bookingCode){
  try {
    const stream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment' } });
    _scannerStream = stream;
    const modal = document.createElement('div');
    modal.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,.85);z-index:1000;display:flex;align-items:center;justify-content:center;padding:20px';
    modal.innerHTML = `<div style="background:var(--bg);border-radius:16px;padding:20px;max-width:420px;width:100%">
      <div style="font-weight:700;margin-bottom:8px;text-align:center">📷 QR-Code scannen</div>
      <video style="width:100%;border-radius:12px" playsinline></video>
      <div style="text-align:center;color:var(--muted);font-size:12px;margin-top:6px">Halte die Kamera auf den QR-Code im Laden</div>
      <button class="btn sec2" style="margin-top:12px;width:100%" onclick="AF.closeScanner()">Abbrechen</button></div>`;
      ...
      _scannerInterval = setInterval(async () => {
        try {
          ...
          if (codes.length > 0){
            ...
            let shopCode = '';
            try { const u = new URL(qrData); shopCode = u.searchParams.get('c') || ''; } catch { shopCode = qrData.trim().toUpperCase(); }
            if (shopCode){
              closeScanner();
              await checkin(bookingCode, shopCode);
            }
          }
        } catch {}
      }, 250);
    } else {
      // Fallback: BarcodeDetector nicht unterstützt (ältere Browser/iOS)
      closeScanner();
      toast('QR-Scanner wird auf diesem Gerät nicht unterstützt — bitte den Barber, dich über sein Dashboard einzuchecken', true);
    }
  } catch(e){
    toast('Kamera nicht verfügbar — bitte den Barber, dich über sein Dashboard einzuchecken', true);
  }
}
function closeScanner(){
  ...
  if (_scannerModal){ _scannerModal.remove(); _scannerModal = null; }
}

async function checkin(code, shopCodeFromUrl){
  const sc = shopCodeFromUrl || ($('#scode') ? $('#scode').value.trim().toUpperCase() : '');
  if (!sc){ toast('Bitte den Code vom Aushang im Laden eingeben (nicht deinen Buchungscode)', true); return; }
  try { const { error } = await sb.rpc('checkin_booking', { p_code: code, p_shop_code: sc }); if (error) throw error; toast('✓ Angekommen — dein Moment beginnt!'); history.replaceState(null,'',location.pathname + location.hash); viewBooking(code); }
  catch(e){ 
    let msg = errMsg(e);
    if (msg.includes('Falscher Shop-Code')) msg = 'Das ist nicht der richtige Code. Du brauchst den Code vom Aushang/QR-Poster im Laden — nicht deinen Buchungscode.';
    const box = $('#cerr'); if (box) box.innerHTML = `<div class="err">${esc(msg)}</div>`; else toast(msg, true); 
  }
}
...
// =====================================================================
let myStaff = null; // {shop_id, staff_index, name, status, can_view_shop_stats, can_manage_all_bookings} wenn als Mitarbeiter:in angemeldet
let myStaffInvite = null; // gesetzte, aber noch nicht aktivierte Einladung (Passwort noch nicht gesetzt)
async function loadSession(){
  const { data } = await sb.auth.getSession(); session = data.session || null;
  isAdmin = false; myShop = null; myStaff = null; myStaffInvite = null;
  if (session){
    ...
    myShop = s || null;
    if (!myShop){
      // Kein Inhaber — prüfen, ob eine aktive Mitarbeiter-Rolle vorliegt
      try {
        const { data: roleRows } = await sb.rpc('my_staff_role');
        const role = roleRows && roleRows[0];
        if (role){
          myStaff = role;
          const { data: shopForStaff } = await sb.from('shops').select('*').eq('id', role.shop_id).maybeSingle();
          myShop = shopForStaff || null;
        }
      } catch {}
      if (!myStaff){
        // Noch keine aktive Rolle — evtl. eine offene Einladung (Passwort noch nicht gesetzt)?
        try {
          const { data: inv } = await sb.from('staff_members').select('*').eq('user_id', session.user.id).eq('status','invited').maybeSingle();
          if (inv) myStaffInvite = inv;
        } catch {}
      }
    }
  }
...
async function viewBarber(){
  if (!session){
    const errBox = authError ? `<div class="err">${esc(authError)}${authError.includes('abgelaufen') || authError.includes('anfordern') ? ` <span style="color:var(--gold-l);cursor:pointer;text-decoration:underline" onclick="AF.showResetForm()">Neuen Link anfordern</span>` : ''}</div>` : '';
    app.innerHTML = topbar('Für Barber') + `<div class="view"><div class="hero"><div class="h1" style="font-size:24px">Sag deinen leeren Stühlen Lebewohl.</div><p>Volle Kalender, planbarer Umsatz, keine Diskussionen mehr ums Erscheinen. Du machst die Haare — wir kümmern uns um Buchung, Anzahlung und Auszahlung.</p></div>
      ${errBox}
      ...
      <div style="margin-top:10px;text-align:center"><span style="color:var(--gold-l);cursor:pointer;font-size:12.5px" onclick="AF.showResetForm()">Passwort vergessen?</span></div></div>
      <div class="note">Status: <b>nicht angemeldet</b> · ${APP_VERSION}</div></div>` + bottomNav('barber');
    return;
  }
  authError = null;
  if (myStaffInvite) return viewStaffActivate();
  if (!myShop && isAdmin) return viewAdmin();
...
async function setStaffPassword(){
  const pw = $('#stpw').value, box = $('#sterr');
  box.innerHTML = '';
  if (pw.length < 6){ box.innerHTML = '<div class="err">Passwort braucht mindestens 6 Zeichen.</div>'; return; }
  const btn = $('#stBtn'); btn.disabled = true; btn.innerHTML = '<span class="spinner"></span> Speichere …';
  try {
    const { error: pwErr } = await sb.auth.updateUser({ password: pw });
    if (pwErr) throw pwErr;
    const { error: actErr } = await sb.rpc('activate_my_staff_invite');
    if (actErr) throw actErr;
    await loadSession();
    toast('✓ Willkommen im Team!');
    nav('barber');
  } catch(e){ box.innerHTML = `<div class="err">${esc(errMsg(e))}</div>`; btn.disabled = false; btn.textContent = 'Passwort speichern & loslegen'; }
}
function viewStaffActivate(){
  const inv = myStaffInvite;
  app.innerHTML = topbar('Willkommen im Team') + `<div class="view"><div class="hero" style="padding-top:6px"><div class="h1" style="font-size:22px">Hallo ${esc(inv.name)} 👋</div><p>Du wurdest als Teammitglied eingeladen. Leg jetzt dein eigenes Passwort fest, dann geht's direkt in deinen Kalender.</p></div>
    <label class="l">Passwort wählen (mind. 6 Zeichen)</label><input id="stpw" type="password" placeholder="Passwort" autocomplete="new-password">
    <button class="btn" style="margin-top:12px" id="stBtn" onclick="AF.setStaffPassword()">Passwort speichern &amp; loslegen</button><div id="sterr"></div>
    <div class="note" style="margin-top:14px">Du siehst standardmäßig nur deinen eigenen Kalender und deine eigenen Termine — dein Inhaber kann dir bei Bedarf mehr Rechte geben.</div></div>`;
}
...
function showResetForm(){
  app.innerHTML = topbar('Passwort zurücksetzen', 'barber') + `<div class="view">
    <div class="hero" style="padding-top:6px"><div class="h1" style="font-size:22px">Passwort vergessen?</div><p>Gib deine E-Mail-Adresse ein. Du bekommst einen Link, über den du ein neues Passwort setzen kannst.</p></div>
    <label class="l">E-Mail</label><input id="resetEmail" type="email" placeholder="deine@email.de" autocomplete="email">
    <button class="btn" style="margin-top:12px" id="resetBtn" onclick="AF.sendReset()">Link zum Zurücksetzen senden</button>
    <div id="resetMsg"></div></div>`;
}
async function sendReset(){
  const email = $('#resetEmail').value.trim();
  const box = $('#resetMsg');
  if (!email.includes('@')){ box.innerHTML = '<div class="err">Bitte gültige E-Mail eingeben.</div>'; return; }
  const btn = $('#resetBtn'); btn.disabled = true; btn.innerHTML = '<span class="spinner"></span> Sende …';
  try {
    try { localStorage.setItem('afro_reset_pending', 'true'); } catch {}
    const { error } = await sb.auth.resetPasswordForEmail(email, { redirectTo: location.origin + location.pathname });
    if (error) throw error;
    box.innerHTML = '<div class="lock">✓ E-Mail gesendet! Prüfe dein Postfach (auch Spam). Klicke den Link in der Mail, um dein neues Passwort festzulegen.</div>';
  } catch(e){
    try { localStorage.removeItem('afro_reset_pending'); } catch {}
    box.innerHTML = `<div class="err">${esc(errMsg(e))}${String(errMsg(e)).toLowerCase().includes('rate') ? '<br><br>Zu viele Anfragen — bitte in ein paar Minuten erneut versuchen.' : ''}</div>`;
    btn.disabled = false; btn.textContent = 'Link zum Zurücksetzen senden';
  }
}
...
function renderResetForm(){
  app.innerHTML = topbar('Neues Passwort') + `<div class="view">
    <div class="hero" style="padding-top:6px"><div class="h1" style="font-size:22px">Neues Passwort festlegen</div><p>Wähle ein neues Passwort (mindestens 6 Zeichen).</p></div>
    <label class="l">Neues Passwort</label><input id="newPw" type="password" placeholder="Neues Passwort" autocomplete="new-password">
    <label class="l">Passwort bestätigen</label><input id="newPw2" type="password" placeholder="Nochmal eingeben" autocomplete="new-password">
    <button class="btn" style="margin-top:12px" id="setPwBtn" onclick="AF.confirmReset()">Passwort speichern</button>
    <div id="resetPwMsg"></div></div>`;
}
async function confirmReset(){
  const pw = $('#newPw').value, pw2 = $('#newPw2').value;
  const box = $('#resetPwMsg');
  if (pw.length < 6){ box.innerHTML = '<div class="err">Passwort braucht mindestens 6 Zeichen.</div>'; return; }
  if (pw !== pw2){ box.innerHTML = '<div class="err">Die Passwörter stimmen nicht überein.</div>'; return; }
  const btn = $('#setPwBtn'); btn.disabled = true; btn.innerHTML = '<span class="spinner"></span> Speichere …';
  try {
    const { error } = await sb.auth.updateUser({ password: pw });
    if (error) throw error;
    box.innerHTML = '<div class="lock">✓ Passwort gespeichert! Du kannst dich jetzt mit deinem neuen Passwort anmelden.</div>';
    setTimeout(() => nav('barber'), 2500);
  } catch(e){
    box.innerHTML = `<div class="err">${esc(errMsg(e))}</div>`;
...
function renderForm(edit){
  ...
  app.innerHTML = topbar(edit ? 'Shop bearbeiten' : 'Partner werden', edit ? 'barber' : null, edit ? '' : `<span class="pill" onclick="AF.logout()" style="cursor:pointer">Abmelden</span>`) + `<div class="view">
    ...
    <div class="sec">Öffnungszeiten</div>
    <div class="note" style="margin-bottom:8px">${f.type==='mobile' ? 'Gib hier an, an welchen Tagen du <b>grundsätzlich</b> verfügbar bist — dein festes Wochengerüst. Einzelne Tage oder Stunden sperrst du später im Dashboard unter „Verfügbarkeit" (z. B. Urlaub, Arzttermin). Sonntag ist als Ruhetag voreingestellt.' : 'Das feste Grundgerüst deines Ladens. Tagesgenaue Anpassungen (z. B. Urlaub) machst du im Dashboard unter „Verfügbarkeit".'}<br>🌙 Sonntage und gesetzliche Feiertage werden automatisch gesperrt — du musst sie nicht manuell eintragen.</div>
    <div class="card" style="padding:4px 14px">${hoursRows}</div>
...
async function viewDash(tab){
  ...
  const tabs = ['today','avail','stats','code','more'].map(k => `<div class="it ${tab===k?'on':''}" onclick="AF.dash('${k}')"><div>${{today:'📊',avail:'🗓️',stats:'📈',code:'🔳',more:'⚙️'}[k]}</div>${{today:'Termine',avail:'Verfügbarkeit',stats:'Auswertung',code:'Shop-Code',more:'Mehr'}[k]}</div>`).join('')
    + `<div class="it logout-it" onclick="AF.logout()"><div>🚪</div>Abmelden</div>`;
...
async function dashToday(statusBar, navHtml){
  ...
  const staffBanner = myStaff ? `<div class="lock" style="margin-bottom:10px">👤 Angemeldet als <b>${esc(myStaff.name)}</b>${myStaff.can_manage_all_bookings ? ' — siehst alle Termine des Teams' : ' — du siehst nur deine eigenen Termine'}</div>` : '';
  app.innerHTML = topbar('', null, `<span class="pill">${esc(s.name)}</span>`) + `<div class="view">${statusBar}${staffBanner}
  ...
  try {
    ...
    const cards = list.length ? list.map(b => {
      const st = STATUS_BARBER[b.status] || [b.status,''];
      ...
      const [bh,bm] = b.time.split(':').map(Number); const bStart = new Date(fromKey(b.date)); bStart.setHours(bh,bm,0,0);
      const canNoShow = new Date() >= new Date(bStart.getTime() + 15*60000);
      const noShowBtn = canNoShow ? `<button class="btn small sec2" onclick="AF.markNoShow('${b.id}')">Nicht erschienen</button>` : `<span class="note" style="font-size:11px">No-Show ab ${pad(bStart.getHours())}:${pad(bStart.getMinutes()+15)} Uhr</span>`;
      const acts = b.status==='booked' ? `${arriveBtn}<button class="btn small ${myShop.type==='mobile'?'sec2':'ok'}" onclick="AF.setStatus('${b.id}','checked_in')">✓ Kunde ist da</button>${noShowBtn}<button class="btn small danger" onclick="AF.setStatus('${b.id}','cancelled_barber')">Ich muss absagen</button>` : b.status==='checked_in' ? `<button class="btn small ok" onclick="AF.setStatus('${b.id}','completed')">Termin abgeschlossen</button>` : '';
      ...
      return `<div class="bk"><div class="top"><div><span class="tm">${b.time}</span> <span class="badge ${st[1]}">${st[0]}</span><div style="font-weight:700;margin-top:4px">${esc(b.customer_name)}</div><div class="meta">${esc(b.service_name)} · ${b.duration} min · ${eur(b.price)}${b.staff_name?' · '+esc(b.staff_name):''}</div>${b.customer_address ? `<div class="meta">🏠 ${esc(b.customer_address)}</div>` : ''}<div class="meta" style="color:var(--gold-l)">💬 Kontakt nur über den Chat — keine Telefonnummer hinterlegt</div></div></div><div class="acts">${acts}${dep}${chatBtn}</div></div>`;
...
async function markNoShow(id){
  if (!confirm('Kunde wirklich als „Nicht erschienen" markieren? Die Anzahlung verbleibt bei dir.')) return;
  try {
    const { error } = await sb.rpc('mark_no_show', { p_booking_id: id });
    if (error) throw error;
    toast('✓ No-Show vermerkt'); viewDash('today');
  } catch(e){ toast(errMsg(e), true); }
}
...
// ---------- Verfügbarkeit (nächste 14 Tage × Friseur × Stunde, passend zum Buchungsfenster) ----------
let avIdx = 0;
async function dashAvail(statusBar, navHtml){
  const s = myShop; const staff = s.staff?.length ? s.staff : [{name:'Ich'}];
  const dates = Array.from({length: 14}, (_,i) => i).map(i => { const d = today(); d.setDate(d.getDate()+i); return d; });
  ...
  let grid = '';
  if (!h) grid = '<div class="warnbox">🔔 Dieser Wochentag ist in deinen Öffnungszeiten als geschlossen eingetragen — Kunden können nichts buchen. Falls du das ändern willst: „Mehr" → „Shop bearbeiten" → Öffnungszeiten.</div>';
  const hName = holidayName(dates[avIdx]);
  if (hName) grid = `<div class="warnbox">📅 <b>${esc(hName)}</b> — gesetzlicher Feiertag, automatisch gesperrt. Kunden können an diesem Tag nicht buchen.</div>`;
  else {
  ...
  app.innerHTML = topbar('Verfügbarkeit') + `<div class="view">${statusBar}<div class="note" style="margin:0 0 10px">Rot = bereits gebuchter Termin (mit Kundenname). Tippe eine freie Stunde an, um sie zu sperren — oder den Namen oben, um den ganzen Tag freizustellen (Urlaub/Ausfall).</div>
    <div class="tabs">${dates.map((x,i)=>`<div class="tab ${i===avIdx?'on':''} ${!hoursFor(s,x)||isHoliday(x)?'closed':''}" onclick="AF.avTab(${i})">${DOW2[x.getDay()]}<span class="d">${x.getDate()}.${x.getMonth()+1}.</span>${isHoliday(x)?'<span class="dot" title="Feiertag"></span>':(!cover[i]?'<span class="dot"></span>':'')}</div>`).join('')}</div>${grid}
...
async function saveUnavail(){ 
  const { error } = await sb.from('shops').update({ unavail: myShop.unavail }).eq('id', myShop.id); 
  if (error){ toast(errMsg(error), true); return false; } 
  const { data: updated } = await sb.from('shops').select('*').eq('id', myShop.id).single();
  if (updated) myShop = updated;
  return true; 
}
async function togHour(si, dk, hh){ const u = myShop.unavail = myShop.unavail || {}; u[si] = u[si] || {}; const arr = u[si][dk] = u[si][dk] || []; const i = arr.indexOf(hh); if (i>=0) arr.splice(i,1); else arr.push(hh); const ok = await saveUnavail(); if (ok) toast('✓ Gespeichert'); viewDash('avail'); }
async function togDay(si, dk, hrs){
  const u = myShop.unavail = myShop.unavail || {}; u[si] = u[si] || {};
  const cur = u[si][dk] || [];
  const wasOff = hrs.every(h => cur.includes(h));
  const turningOff = !wasOff; // wir schalten den Tag gerade AUS (nicht wieder frei)
  const hadExisting = turningOff && await (async () => {
    try {
      const { data: existing } = await sb.from('bookings').select('id,customer_name,time').eq('shop_id', myShop.id).eq('staff_index', si).eq('date', dk).eq('status', 'booked');
      if (existing && existing.length){
        const liste = existing.map(b => b.time + ' Uhr — ' + b.customer_name).join('\n');
        const alsoCancel = confirm(`An diesem Tag gibt es bereits ${existing.length} Buchung(en):\n${liste}\n\nSollen diese ebenfalls abgesagt werden? Kunden werden benachrichtigt und die Anzahlung wird erstattet.`);
        if (alsoCancel){
          for (const b of existing){ await sb.from('bookings').update({ status: 'cancelled_barber' }).eq('id', b.id); }
          toast(`✓ ${existing.length} Termin(e) abgesagt`);
        } else {
          toast('Tag freigenommen — bestehende Termine bleiben unverändert bestehen', true);
        }
        return true;
      }
      return false;
  ...
  u[si][dk] = turningOff ? [...hrs] : [];
  const ok = await saveUnavail();
  if (ok && !hadExisting) toast('✓ Gespeichert');
  viewDash('avail');
}
...
function dashMore(navHtml){
  ...
  if (myStaff){
    app.innerHTML = topbar('Mehr') + `<div class="view">
      <div class="card"><div class="name">👤 ${esc(myStaff.name)}</div><div class="meta">Teammitglied bei ${esc(s.name)}</div></div>
      <div class="card"><div class="name">Meine Rechte</div><div class="meta">${myStaff.can_manage_all_bookings ? '✓ Sieht/verwaltet Termine des ganzen Teams' : 'Nur eigene Termine'}${myStaff.can_view_shop_stats ? ' · ✓ Sieht Auswertung des ganzen Shops' : ''}</div><div class="note">Möchtest du mehr Rechte? Sprich mit deinem Inhaber/deiner Inhaberin.</div></div>
  ...
  app.innerHTML = topbar('Mehr') + `<div class="view">
    ...
    ${(s.staff||[]).length > 0 ? `<div class="card click" onclick="location.hash='#/team'"><div class="name">👥 Team verwalten</div><div class="meta">Mitarbeiter einladen und Rechte vergeben</div></div>` : `<div class="card click" onclick="AF.editShop()"><div class="name">👥 Team hinzufügen</div><div class="meta">Friseure eintragen, dann einladen — jede:r bekommt einen eigenen Login</div></div>`}
    ${stripeCard}
...
async function viewTeam(){
  if (!myShop || myStaff){ return nav('barber'); }
  const s = myShop;
  app.innerHTML = topbar('Team verwalten', 'barber') + '<div class="view"><span class="spinner"></span> Lade …</div>';
  try {
    const { data: rows, error } = await sb.from('staff_members').select('*').eq('shop_id', s.id).order('staff_index');
    if (error) throw error;
    const byIndex = {}; (rows||[]).forEach(r => byIndex[r.staff_index] = r);
    const cards = (s.staff||[]).map((x, i) => {
      const st = byIndex[i];
      const statusBadge = !st ? '<span class="badge">Nicht eingeladen</span>' : st.status==='active' ? '<span class="badge">✓ Aktiv</span>' : '<span class="badge warn">⏳ Einladung offen</span>';
      const inviteForm = !st ? `<div class="row" style="margin-top:8px"><input id="temail${i}" placeholder="E-Mail von ${esc(x.name)}" type="email"><button class="btn small" onclick="AF.inviteStaff(${i},'${esc(x.name).replace(/'/g,"\\'")}')">Einladen</button></div><div id="tinv${i}"></div>` : '';
      const perms = st ? `<div class="acts" style="margin-top:8px">
          <label class="chip ${st.can_view_shop_stats?'on':''}" style="cursor:pointer" onclick="AF.toggleStaffPerm('${st.id}','can_view_shop_stats',${!st.can_view_shop_stats})">📈 Ganze Auswertung</label>
          <label class="chip ${st.can_manage_all_bookings?'on':''}" style="cursor:pointer" onclick="AF.toggleStaffPerm('${st.id}','can_manage_all_bookings',${!st.can_manage_all_bookings})">📅 Alle Termine</label>
        </div>` : '';
      return `<div class="bk"><div class="top"><div><div style="font-weight:700">${esc(x.name)}</div><div class="meta">${st ? esc(st.email) : 'Noch keine E-Mail hinterlegt'}</div></div>${statusBadge}</div>${inviteForm}${perms}</div>`;
    }).join('');
    app.innerHTML = topbar('Team verwalten', 'barber') + `<div class="view">
      <div class="note" style="margin-bottom:10px">Lade Teammitglieder per E-Mail ein. Sie bekommen einen Link, setzen ihr eigenes Passwort und sehen danach standardmäßig nur ihren eigenen Kalender. Mehr Rechte gibst du hier gezielt frei.</div>
      ${cards || '<div class="note">Noch kein Team hinterlegt — trag Namen unter „Shop bearbeiten“ → Team ein.</div>'}
      <div style="height:20px"></div></div>`;
  } catch(e){ app.innerHTML = topbar('Team verwalten', 'barber') + `<div class="view"><div class="err">${esc(errMsg(e))}</div></div>`; }
}
async function inviteStaff(staffIndex, name){
  const email = $('#temail'+staffIndex).value.trim(), box = $('#tinv'+staffIndex);
  if (!email.includes('@')){ box.innerHTML = '<div class="err">Bitte gültige E-Mail eingeben.</div>'; return; }
  box.innerHTML = '<span class="spinner"></span> Sende Einladung …';
  try {
    await callEdge('invite-staff', { shop_id: myShop.id, staff_index: staffIndex, name, email, return_base: location.origin + location.pathname }, true);
    toast('✓ Einladung gesendet'); viewTeam();
  } catch(e){ box.innerHTML = `<div class="err">${esc(errMsg(e))}</div>`; }
}
async function toggleStaffPerm(rowId, field, value){
  const { error } = await sb.from('staff_members').update({ [field]: value }).eq('id', rowId);
  if (error) return toast(errMsg(error), true);
  toast('✓ Aktualisiert'); viewTeam();
}
...
// =====================================================================
let displayTimer = null, chatTimer = null, heroTimer = null;
...
async function viewAdmin(tab){
  ...
  try {
    ...
    const tabs = ['overview','barbers','bookings','payments','approvals'].map(k => `<div class="tab ${adminTab===k?'on':''}" onclick="AF.adminTab('${k}')">${{overview:'📊 Übersicht',barbers:'💈 Barber',bookings:'📅 Termine',payments:'💶 Zahlungen',approvals:'🛡️ Freigaben'}[k]}</div>`).join('')
      + `<div class="tab logout-it" onclick="AF.logout()">🚪 Abmelden</div>`;
    let body = '';
...
function adminBookings(){
  ...
  return rangeSwitch() +
    ...
    (list.length ? list.slice(0,200).map(b => {
      ...
      return `<div class="bk"><div class="top"><div><span class="tm">${b.date} · ${b.time}</span><div style="font-weight:700;margin-top:3px">${esc(b.customer_name)} · <span style="font-weight:400;color:var(--muted)">${esc(b.shops?.name||'')}</span></div><div class="meta">${esc(b.service_name)} · ${eur(b.price)} · Anzahlung ${eur(b.deposit_amount)} (${b.deposit_status==='paid'?'bezahlt':b.deposit_status==='refunded'?'erstattet':'offen'})</div></div><span class="badge ${badge}">${ST_LABEL[b.status]||b.status}</span></div></div>`;
...
async function render(){
  ...
  if (parts[0] !== 'home' && parts[0] !== ''){ clearInterval(heroTimer); }
  try {
    ...
    if (parts[0]==='my'){
      ...
      const scFromUrl = urlParams.get('c');
      if (scFromUrl){
        const codes = myCodes();
        if (codes.length){
          // Gespeicherte Buchungscodes vorhanden → automatisch versuchen
          history.replaceState(null, '', location.pathname + '#/my');
          let checkedIn = false;
          let lastError = '';
          for (const saved of codes){
            try {
              const { error } = await sb.rpc('checkin_booking', { p_code: saved.code, p_shop_code: scFromUrl.toUpperCase() });
              if (!error){ toast('✓ Angekommen — dein Moment beginnt!'); nav('b/' + saved.code); checkedIn = true; break; }
              else { lastError = error.message || String(error); }
            } catch(e){ lastError = errMsg(e); }
          }
          if (!checkedIn){ viewMy(); toast(lastError || 'Kein offener Termin für diesen Shop gefunden', true); }
          return;
        } else {
          // Keine gespeicherten Codes → Kunde muss Buchungscode eingeben
          app.innerHTML = topbar('Einchecken') + `<div class="view">
            <div class="warnbox">📍 Du hast den QR-Code gescannt — jetzt noch deinen <b>Buchungscode</b> eingeben (den 6-stelligen Code, den du bei der Buchung bekommen hast).</div>
            <label class="l" style="margin-top:12px">Dein Buchungscode</label>
            <div class="row"><input id="qrBookCode" placeholder="z. B. A1B2C3" style="text-transform:uppercase" maxlength="6"><button class="btn small" onclick="AF.qrCheckin('${esc(scFromUrl)}')">Einchecken</button></div>
            <div id="qrErr"></div>
            <div class="note" style="margin-top:12px">Du findest den Code in deiner Buchungsbestätigung (E-Mail oder Screenshot).</div></div>`;
          return;
        }
      }
      return viewMy();
    }
    if (parts[0]==='b'){ const gm = (q||'').match(/group=(\d+)/); return viewBooking(parts[1], (q||'').includes('new=1'), (q||'').includes('paid=1'), gm ? +gm[1] : 0); }
    if (parts[0]==='barber') return viewBarber();
    if (parts[0]==='reset-password') return viewResetPassword();
    if (parts[0]==='bchat') return viewBarberChat(parts[1]);
    if (parts[0]==='team') return viewTeam();
...
window.AF = { pickService, afterService, pickStaff, filterStaff, setQty, day, pickTime, book, openScanner, closeScanner, checkin, cancel, dispute, sendLink, pwLogin, signUp, logout, showResetForm, sendReset, confirmReset, f: fset, rf, togSpec, addArea, sv, addSv, rmSv, st, addSt, rmSt, saveShop, dash: viewDash, dday, setStatus, depPaid, markNoShow, arrived, avTab, togHour, togDay, editShop, connectStripe, shopStatus, resolve, toast, setCity, useLocation, sendChatCustomer, sendChatBarber, statsRange: statsSetRange, statsSearch, adminTab: adminSetTab, adminRange: adminSetRange, adminSearch, setStaffPassword, inviteStaff, toggleStaffPerm };

async function handleAuthRedirect(){
  const h = location.hash || '';
  const search = location.search || '';

  // ---- Weg 1: Token als Query-Parameter (aus dem neuen E-Mail-Template) ----
  const qp = new URLSearchParams(search);
  const otpToken = qp.get('token');
  const otpType = qp.get('type');
  const otpEmail = qp.get('email');
  if (otpToken && otpType === 'recovery'){
    try {
      const { data, error } = await sb.auth.verifyOtp({ email: otpEmail, token: otpToken, type: 'recovery' });
      if (error){ authError = 'Link ungültig oder abgelaufen: ' + error.message; history.replaceState(null, '', location.pathname + '#/barber'); return true; }
      await loadSession();
      try { localStorage.removeItem('afro_reset_pending'); } catch {}
      history.replaceState(null, '', location.pathname + '#/reset-password');
    } catch(e){ authError = 'Fehler beim Zurücksetzen: ' + errMsg(e); history.replaceState(null, '', location.pathname + '#/barber'); }
    return true;
  }

  // ---- Weg 2: Fehler von Supabase im Hash abfangen (z. B. abgelaufener Link) ----
  if (h.includes('error=')){
    const ep = new URLSearchParams(h.substring(h.indexOf('error=')));
    const errCode = ep.get('error_code') || '';
    const errDesc = (ep.get('error_description') || '').replace(/\+/g, ' ');
    if (errCode === 'otp_expired' || errDesc.toLowerCase().includes('expired')){
      authError = 'Der Link ist abgelaufen. Bitte fordere einen neuen an.';
    } else {
      authError = errDesc || 'Ein Fehler ist aufgetreten. Bitte versuche es erneut.';
    }
    history.replaceState(null, '', location.pathname + '#/barber');
    return true;
  }

  // ---- Weg 3: Impliziter Flow (access_token im Hash-Fragment) ----
  ...
  try {
    ...
    if (error){ authError = 'Login fehlgeschlagen: ' + error.message; return true; }
    if (!data || !data.session){ authError = 'Login-Token wurde nicht akzeptiert.'; return true; }
    await loadSession();
  } catch(e){ authError = 'Login-Fehler: ' + errMsg(e); return true; }
  const isRecovery = type === 'recovery' || (() => { try { return localStorage.getItem('afro_reset_pending') === 'true'; } catch { return false; } })();
  if (isRecovery){
    try { localStorage.removeItem('afro_reset_pending'); } catch {}
    history.replaceState(null, '', location.pathname + '#/reset-password');
  } else {
...
(async function boot(){
  ...
  sb.auth.onAuthStateChange(async (ev) => {
    if (ev === 'PASSWORD_RECOVERY' || (ev === 'SIGNED_IN' && (() => { try { return localStorage.getItem('afro_reset_pending') === 'true'; } catch { return false; } })())){
      await loadSession();
      try { localStorage.removeItem('afro_reset_pending'); } catch {}
      if (session){
    ...
    if (ev === 'SIGNED_IN' || ev === 'SIGNED_OUT' || ev === 'TOKEN_REFRESHED'){
      await loadSession();
      // Wenn wir auf der Reset-Seite sind, dort bleiben — nicht wegnavigieren
      if (location.hash === '#/reset-password') return;
      if (route().startsWith('barber') || route().startsWith('admin')) render();
    }
  });
  // Direkt nach Login-Redirect: bei Reset zur Passwort-Seite, sonst zum Barber-Dashboard
  if (didAuth && session){
    if (location.hash === '#/reset-password' || (() => { try { return localStorage.getItem('afro_reset_pending') === 'true'; } catch { return false; } })()){
      try { localStorage.removeItem('afro_reset_pending'); } catch {}
      location.hash = '#/reset-password';
    } else {
      location.hash = '#/barber';
    }
  }
