  ...
  try {
    ...
    app.innerHTML = topbar('Deine Buchung', 'my') + `<div class="view">
      ...
      ${b.status==='booked' && s.type!=='mobile' ? `<div class="card">
        ...
        <div class="note" style="margin-top:0">Scanne den QR-Code an der Rezeption — oder tippe den Code vom Aushang ein.</div>
        <div class="row" style="margin-top:10px;gap:6px">
          <button class="btn small" ${canCheckin?'':'disabled'} onclick="AF.openScanner('${b.code}')">📷 QR-Code scannen</button>
          <input id="scode" placeholder="oder Code eintippen" style="flex:1;text-transform:uppercase" value="${(() => { try { return new URLSearchParams(location.search).get('c') || ''; } catch { return ''; } })()}" ${canCheckin?'':'disabled'}>
          <button class="btn small sec2" ${canCheckin?'':'disabled'} onclick="AF.checkin('${b.code}')">OK</button>
        </div>
...
async function openScanner(bookingCode){
  try {
    ...
    if ('BarcodeDetector' in window){
      ...
      closeScanner();
      toast('QR-Scanner wird auf diesem Gerät nicht unterstützt — bitte Code manuell eintippen', true);
    }
  } catch(e){
    toast('Kamera nicht verfügbar — bitte Code manuell eintippen', true);
  }
}
...
window.AF = { pickService, afterService, pickStaff, filterStaff, setQty, day, pickTime, book, openScanner, closeScanner, checkin, cancel, dispute, sendLink, pwLogin, signUp, logout, showResetForm, sendReset, confirmReset, f: fset, rf, togSpec, addArea, sv, addSv, rmSv, st, addSt, rmSt, saveShop, dash: viewDash, dday, setStatus, depPaid, arrived, avTab, togHour, togDay, editShop, connectStripe, shopStatus, resolve, toast, setCity, useLocation, sendChatCustomer, sendChatBarber, statsRange: statsSetRange, statsSearch, adminTab: adminSetTab, adminRange: adminSetRange, adminSearch, setStaffPassword, inviteStaff, toggleStaffPerm };
