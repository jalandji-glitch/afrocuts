...
// ---------- Supabase ----------
let sb = null, session = null, isAdmin = false, myShop = null;
function initSb(){
...
function topbar(title, back, right){
  return `<div class="topbar">${back ? `<div class="back" onclick="location.hash='#/${back}'">←</div>` : `<div class="logo">AFROCUTS${C.TEST_MODE ? '<span class="testtag">TEST</span>' : ''}</div>`}
...
async function viewHome(){
  app.innerHTML = topbar() + `<div class="view"><div class="hero"><div class="h1">Afro-Barber in deiner Nähe</div><p>Termin wählen, Anzahlung sichern, entspannt hingehen. Ohne Anmeldung.</p></div><div id="list"><span class="spinner"></span> Lade Barber …</div></div>` + bottomNav('home');
  try {
...
async function viewBarber(){
  if (!session){
    app.innerHTML = topbar('Für Barber') + `<div class="view"><div class="hero"><div class="h1" style="font-size:24px">Werde AFROCUTS-Partner</div><p>Du bestimmst Kalender, Preise und Anzahlung. Kunden buchen verbindlich — weniger No-Shows.</p></div>
      <div class="card"><div style="font-weight:700;margin-bottom:8px">Anmelden oder registrieren</div><div class="note" style="margin:0 0 10px">Wir schicken dir einen Login-Link per E-Mail — kein Passwort nötig.</div>
      <input id="email" type="email" placeholder="deine@email.de" autocomplete="email"><button class="btn" style="margin-top:10px" id="otpBtn" onclick="AF.sendLink()">Login-Link senden</button><div id="oerr"></div></div>
      <div class="note">Nach dem Klick auf den Link in der Mail landest du automatisch hier zurück.</div></div>` + bottomNav('barber');
    return;
  }
...
window.AF = { pickService, afterService, pickStaff, day, pickTime, book, checkin, cancel, dispute, sendLink, logout, f: fset, rf, togSpec, addArea, sv, addSv, rmSv, st, addSt, rmSt, saveShop, dash: viewDash, dday, setStatus, depPaid, avTab, togHour, togDay, editShop, shopStatus, resolve, toast };

async function handleAuthRedirect(){
  // Supabase hängt den Login-Token als #access_token=... an — bei uns entsteht dadurch ein zweites '#'.
  // Wir fischen den Token-Teil heraus, geben ihn an Supabase und säubern die Adresse.
  const h = location.hash || '';
  const at = h.indexOf('access_token=');
  if (at === -1) return false;
  const tokenStr = h.substring(at);
  const p = new URLSearchParams(tokenStr);
  const access_token = p.get('access_token');
  const refresh_token = p.get('refresh_token');
  // Route retten, die vor dem Token stand (z.B. #/barber#access_token=... -> "barber")
  let target = 'barber';
  const before = h.substring(0, at).replace(/^#\/?/, '').replace(/#$/, '');
  if (before) target = before;
  if (access_token && refresh_token){
    try { await sb.auth.setSession({ access_token, refresh_token }); } catch(e){ /* ignore */ }
  }
  // Adresse säubern: nur noch eine saubere Route, kein Token mehr sichtbar
...
(async function boot(){
  ...
  await loadSession();
  sb.auth.onAuthStateChange(async (ev) => { if (ev === 'SIGNED_IN' || ev === 'SIGNED_OUT' || ev === 'TOKEN_REFRESHED'){ await loadSession(); if (route().startsWith('barber') || route().startsWith('admin')) render(); } });
