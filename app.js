...
async function viewHome(){
  ...
  app.innerHTML = topbar() + `<div class="view"><div class="hero"><div class="h1">Afro-Barber in deiner Nähe</div><p>Termin wählen, Anzahlung sichern, entspannt hingehen. Ohne Anmeldung.</p></div>${loggedInBanner}
    <label class="l">📍 In welcher Stadt suchst du?</label><div class="row"><input id="cityIn" placeholder="z. B. Berlin, Köln, Hamburg …" value="${esc(cityFilter)}"><button class="btn small" onclick="AF.setCity(document.getElementById('cityIn').value)">Suchen</button></div>
  ...
  try {
    ...
    list.innerHTML = '<div class="sec">Verfügbare Barber</div>' + filtered.map(s => `<div class="card click" onclick="location.hash='#/shop/${s.id}'"><div class="row"><div class="thumb">${s.type==='mobile'?'🚗':'💈'}</div><div style="flex:1;min-width:0"><div class="name">${esc(s.name)}</div><div class="meta">${esc(shopMeta(s))}</div><span class="badge">✓ Direktbuchung</span></div><div style="color:var(--muted)">›</div></div></div>`).join('');
...
window.AF = { pickService, afterService, pickStaff, day, pickTime, book, checkin, cancel, dispute, sendLink, pwLogin, signUp, logout, f: fset, rf, togSpec, addArea, sv, addSv, rmSv, st, addSt, rmSt, saveShop, dash: viewDash, dday, setStatus, depPaid, arrived, avTab, togHour, togDay, editShop, shopStatus, resolve, toast, setCity, adminTab: adminSetTab, adminRange: adminSetRange, adminSearch };
