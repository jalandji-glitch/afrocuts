...
function renderTimeView(){
  ...
  if (B._per === null){
    ...
    if (!times.length){
      slotsHtml = `<div class="empty">📭 Keine freien Termine an diesem Tag${B.staff!=='any' && multi ? ' bei dieser Person' : ''}<br>${B.staff!=='any' && multi ? 'Tippe oben „Beliebig" oder einen anderen Namen — oder schau beim nächsten Tag vorbei' : 'Schau beim nächsten Tag vorbei'}</div>`;
    } else {
      slotsHtml = times.map(t => {
        ...
        return `<div class="opt ${B.time===t?'on':''}" onclick="AF.pickTime('${t}')">${t}${sn && multi ? `<span style="display:block;font-size:9.5px;opacity:.7;font-weight:400">${esc(sn)}</span>` : ''}</div>`;
      }).join('');
    }
  }
  box.innerHTML = `${heroBanner('Wann passt es dir?', 'Freie Slots gehen schnell weg — schnapp dir deinen.', B._heroImg)}
    ...
    <div class="sec">Freie Uhrzeiten</div><p style="color:var(--muted);font-size:12px;margin:-4px 0 8px">Jeder freie Slot ist echt — sichere ihn dir, bevor es jemand anders tut.</p><div class="slots" id="slots">${slotsHtml}</div>
    <div class="summary" id="sum"><b>${esc(B.shop.name)}</b> · ${esc(B.service.name)} · ${B.service.duration} min<br>${B.time ? fmtD(d) + ' · ' + B.time + ' Uhr' + (multi && B.shop.staff[B.staffIdx]?.name ? ' · bei ' + esc(B.shop.staff[B.staffIdx].name) : '') : 'Wähle Tag &amp; Uhrzeit'}</div><div style="height:80px"></div>
...
function viewPay(){
  ...
  app.innerHTML = topbar('Anzahlung', 'time') + `<div class="view">
    ...
    ${B.shop.type==='mobile' ? `<div class="note" style="color:var(--gold-l);margin-bottom:2px">Kein Warten mit Kindern im Laden — du bekommst deine Zeit zurück.</div><div class="sec">🚗 Deine Adresse für den Hausbesuch</div><input id="addr" placeholder="Straße, Hausnummer, PLZ, Etage">` : ''}
    ...
    <label class="l">Handynummer oder E-Mail</label><input id="ccontact" placeholder="Für Buchungsbestätigung — der Barber schreibt dir über den Chat" autocomplete="tel">
...
async function viewBooking(code, isNew, justPaid){
  ...
  try {
    ...
    app.innerHTML = topbar('Deine Buchung', 'my') + `<div class="view">
      ...
      ${isNew ? '<div class="check">✓</div><div class="center h2">Gesichert. Dein Moment gehört dir.</div><div class="center note" style="margin-bottom:14px">Merk dir deinen Code — damit findest du die Buchung auf jedem Gerät wieder.</div>' : ''}
...
window.AF = { pickService, afterService, pickStaff, filterStaff, day, pickTime, book, checkin, cancel, dispute, sendLink, pwLogin, signUp, logout, f: fset, rf, togSpec, addArea, sv, addSv, rmSv, st, addSt, rmSt, saveShop, dash: viewDash, dday, setStatus, depPaid, arrived, avTab, togHour, togDay, editShop, connectStripe, shopStatus, resolve, toast, setCity, useLocation, sendChatCustomer, sendChatBarber, statsRange: statsSetRange, statsSearch, adminTab: adminSetTab, adminRange: adminSetRange, adminSearch, setStaffPassword, inviteStaff, toggleStaffPerm };
